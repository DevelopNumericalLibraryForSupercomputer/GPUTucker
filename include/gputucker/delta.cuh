...
void ComputingDelta(TensorType *tensor,
  ...
  common::cuda::h2dcpy(dev_bufs.core_idx_addr.get_ptr(0), 
                        h_core_idx_addr, 
  ...
  for (uint64_t iter = 0; iter < tasks.size(); ++iter) {
    ...
    for (int axis = 0; axis < order; ++axis) {
      ...
      common::cuda::h2dcpy_async(dev_bufs.X_indices[axis].get_ptr(stream_offset),
    ...
    gputucker::computing_delta_kernel<index_t, value_t>
        <<<blocks_per_grid, threads_per_block, 0, streams[stream_offset]>>>((std::uintptr_t *)dev_bufs.X_idx_addr.get_ptr(stream_offset),
